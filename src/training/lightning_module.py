...
class FloodModel(pl.LightningModule):
    ...
    def _prep(self, batch: tuple[Tensor, Tensor, Tensor]) -> tuple[Tensor, Tensor, Tensor]:
        img, lbl, mask = batch
        return img, lbl.unsqueeze(1).float(), mask.unsqueeze(1).float()

    def training_step(self, batch: tuple[Tensor, Tensor, Tensor], idx: int) -> Tensor:
    ...
    def validation_step(self, batch: tuple[Tensor, Tensor, Tensor], idx: int) -> None:
        img, target, valid = self._prep(batch)
        logits = self.net(img)
        loss = self.loss(logits, target, valid)
        prob, truth = masked_select(logits, target, valid)
        self.val_metrics.update(prob, truth)
