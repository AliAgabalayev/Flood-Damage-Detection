...
_Run on Colab T4, 40-epoch cap + early stopping (patience 8), seed 42; metrics at
best-`val_iou` epoch on the 89-chip validation split._
...
- **Chosen loss:** **`dice`** (no `pos_weight`) — best on both metrics:
  val IoU **0.6425**, val F1 **0.7824**. Also the simplest (zero imbalance
  hyper-parameters to maintain).
- **Runner-up:** `dice_bce` @ pos_weight 8.4, −0.036 IoU behind. Keep it as the
  fallback: it is the more common default and the gap is small enough that a full
  50-epoch schedule *could* close it.
- **Note vs the team baseline** (`dice_bce`, 50 epochs → IoU 0.65 / F1 0.79):
  our `dice_bce` scored 0.6066 here purely because of the shorter 40-epoch +
  early-stopping schedule — consistent, not a regression. `dice` at 0.6425 is
  already within ~0.01 of that baseline on a shorter schedule.
- **Next actions:**
  1. Set `training.loss: dice` (and `pos_weight: null`) in
     [`config/default.yaml`](../config/default.yaml).
  2. Retrain the winner (and, to be safe, `dice_bce`) at the **full 50 epochs**
     to get the production number and confirm the ranking holds without early
     stopping.
  3. Evaluate the chosen checkpoint on the held-out **test** split:
     `python -m inference.evaluate --split test`.
  4. **Phase 2 (planned):** extend the winning overlap family with an imbalance
     dial and a metric-aligned loss — `focal_tversky` (α<β to boost flood
     recall), `lovasz` (direct IoU surrogate), and `dice_focal`. See the matrix
     note below.
